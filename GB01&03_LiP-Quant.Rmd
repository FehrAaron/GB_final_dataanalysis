...
```{r}
# Combine GB01 and 03_lysate, annotate based on Crapome
...
# Up-Set plot of overlap

upset_plot = combined_prot %>%
  mutate(GB01 = is.na(ProteinLipScore_GB01) == F) %>%
  mutate(GB03 = is.na(ProteinLipScore_GB03) == F) %>%
  upset(intersect = c("GB01", "GB03"))+
...
# Point and correlation plot of shared proteins with scorings

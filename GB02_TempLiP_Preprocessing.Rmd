...
raw_lip1 = read_protti("./Temporal_LiP/GB02_rawfiles/20230412_151616_230404_fehra_GB02_LiP_Report.csv") %>%
...
# preliminary until I have reanalysed the data in SN.

raw_lip = raw_lip1 %>%
  dplyr::filter(grepl("Vort", r_condition) == T | 
                grepl("DMSO_30",r_condition) == T) %>%
  mutate(r_condition = case_when(
    r_condition == "DMSO_30" ~ "Vort_0",
    r_condition == "Vort_30" ~ "trash_6 0",
    r_condition == "Vort_60" ~ "trash_30",
    TRUE ~ r_condition
  )) %>%
  separate(r_condition, into = c("trash","r_condition"), sep = "_") %>%
  mutate(r_condition = paste0(r_condition, "_min")) %>%
...
fwrite(lip_pep, file = paste0("./Temporal_LiP/GB02_Preprocessed_data/",Run,"_",Experiment_Name,"LiP_Peptide_Metadata.csv"))
...
```{r}
raw_trp = read_protti("./Temporal_LiP/GB02_rawfiles/20230416_165343_230404_fehra_GB02_Trp_Report.csv") %>%
  dplyr::filter(grepl("Vort", r_condition) == T | 
                grepl("DMSO_30",r_condition) == T) %>%
  mutate(r_condition = case_when(
    r_condition == "DMSO_30" ~ "Vort_0",

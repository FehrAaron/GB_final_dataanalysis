...
Protein level (show that treatment failed -\> revival of the cells)
...
prot_time = rbind(
  ...
  mutate(time_num = as.numeric(gsub("_min","",time))) %>%
  group_by(pg_protein_accessions) %>%
  mutate(n_sig = sum(adj_pval <= 0.05 & abs(diff) >= 1))
  

prot_time_wide = prot_time %>%
  dplyr::filter(n_sig >= 1) %>%
  pivot_wider(id_cols = c("Gene","pg_protein_accessions"), names_from = "time", values_from = "diff")


cluster_data_trp = prot_time_wide[4:ncol(prot_time_wide)]

# calculate distance matrix and clusters

dist = dist(cluster_data_trp) 

hc_trp = hclust(dist, method = "average")

cluster_trp = cutree(hc_trp, h = 1.5)

plot(hc_trp, main = "Hierarchical clustering of Trp data:",xlab = "", ylab = "", pch = cluster_trp, labels = F)
rect.hclust(hc_lip, k = length(unique(cluster_trp)), cluster = cluster_trp)

# comb with orignial 

trp_clustered = cbind(prot_time_wide, cluster_trp) %>%
  as.data.frame() %>%
  dplyr::select(pg_protein_accessions, cluster = ...15) %>%
  left_join(prot_time) %>%
  group_by(cluster, time) %>%
  mutate(obs_per_cluster = n()) %>%
  mutate(est_centroid = mean(diff))
...
trp_clustered %>%
  ...
  mutate(n = n()/12) %>%
  dplyr::filter(n >= 20) %>%
  dplyr::select(time_num, pg_protein_accessions, cluster, FC = diff, Centroid = est_centroid) %>%
  mutate(Norm = "Log2-FC") %>%
  mutate(clusters = paste0("Cluster ", cluster)) %>%
  ggplot(aes(x = time_num, y = FC, group = pg_protein_accessions)) +
  geom_line(alpha = 0.2)+
